...
void AActorVideoEncoder::ProcessVideoFrame(const FTexture2DRHIRef& FrameBuffer)
{
	...
	if(bFullScreenCapture)
	{
		CopyTexture(FrameBuffer, InputFrameTextureMap[InputFrame]);
		// InputFrameTextureMap[InputFrame] = FrameBuffer;
	}
	else if(SceneCaptureComponent2D != nullptr && SceneCaptureComponent2D->TextureTarget != nullptr)
	{
		CopyTexture(SceneCaptureComponent2D->TextureTarget->GetRenderTargetResource()->GetRenderTargetTexture()
			, InputFrameTextureMap[InputFrame]);
	}
